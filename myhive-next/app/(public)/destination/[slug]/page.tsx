...
export async function generateMetadata({ params }: PageParams): Promise<Metadata> {
  ...
  const title = isPrague
    ? PRAGUE_TITLE
    : `${dest.name} Stag Do — Activities & Packages | Trivlu`;
  const description = isPrague
    ? PRAGUE_DESCRIPTION
    : dest.description ||
      `Everything for a ${dest.name} stag do: top activities, prices for the group and instant trip building.`;
  const url = canonical(`/destination/${slug}`);
  ...
  return {
    ...
    openGraph: {
      ...
      description,
      url,
      ...(dest.imageUrl ? { images: [{ url: dest.imageUrl }] } : {}),
    },
  };
}
