...
админ-логин, полный vote-флоу и букинг против докер-бэкенда); поверх — серия
фиксов по внешнему ревью (OG/robots/404/цены/эскейп-хатч/Markdown/SEO-гейт).
Деплоить текущий head ветки `feat/nextjs-foundation` (хэш не пиним — см.
`git log`; серия фиксов завершена финальным ревью «ready to merge»).
