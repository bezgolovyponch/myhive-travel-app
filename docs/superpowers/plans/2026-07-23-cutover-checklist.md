...
2. **Backend: задеплоить ветки `fix/vote-suggestions-null-bind` и
   ...
   фронтенда) и колонки `seoIndexable` (самодобавляются, prod
   `ddl-auto=update`) — без них поле в ответах API отсутствует, и
   per-record SEO-гейт на фронте работает как «всё не индексируется».
